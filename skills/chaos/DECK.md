...
32. **The page keeps the subject's own secret.** On a writeup *about* someone hiding things in a system and a careful reader finding them, hide one dry line where that exact reader will look — the console — so the medium re-enacts the story. (Landed: a Linux root-compromise post greets anyone who opens devtools with "I knew you'd be the type to look here," then hands them the read-only checklist. Builds on card 30's console-confession, but here the joke *is* the subject: a post about poking systems rewards poking the post.)
